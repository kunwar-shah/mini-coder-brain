...
# Read JSON input from stdin (required by Claude Code statusLine)
input=$(timeout 0.1s cat 2>/dev/null || echo "{}")
...
mkdir -p "$CLAUDE_TMP" 2>/dev/null || true
...
# === BUILD NOTIFICATION (Priority-based) ===
notification_msg=""

# Priority 1: Memory bloat (critical)
if [ -n "$bloat_notification" ]; then
...
elif [ -n "$map_notification" ]; then
  notification_msg="$map_notification"
fi

# === OUTPUT ===
# StatusLine expects SINGLE LINE plain text output (not JSON, not multiple lines)
# Notifications are shown inline with status footer, separated by " | "
if [ -n "$notification_msg" ]; then
  # Status footer + notification inline
  echo "${status_footer} | ${notification_msg}"
else
  # Just status footer
  echo "${status_footer}"
fi
